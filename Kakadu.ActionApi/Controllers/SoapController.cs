...
namespace Kakadu.ActionApi.Controllers
{
    ...
    public class SoapController : /*BaseController,*/ IRequestProcessor
    {
        ...
        public SoapController(ILogger<SoapController> logger, IServiceClient serviceClient) //: base(dataProvider)
        {
            _logger = logger;
        ...
        [Route("{**catchAll}")]
        [HttpPost]
        public async Task ProcessRequest(string serviceCode)
        {
            if(string.IsNullOrWhiteSpace(serviceCode))
                throw new ArgumentNullException(nameof(serviceCode));

            service = await _serviceClient.GetByCodeAsync(serviceCode);
